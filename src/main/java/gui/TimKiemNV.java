...
public class TimKiemNV extends JPanel implements ActionListener {
    ...
    public TimKiemNV() {
        ...
        cboTrinhDo = new JComboBox<>(new String[]{"Tất cả","Đại học", "Cử nhân", "Cao đẳng"});
        gbc.gridx = 7;
        gbc.gridy = 0;
    ...
    private void timKiemNhanVien() {
        ...
        NhanVien nv = new NhanVien(maNV, hoTen, null, gioiTinh, sdt, email, null, trinhDo, null);

        List<NhanVien> dsNV = nvDAO.timKiemNhanVien(nv);

        modelNV.setRowCount(0);
        for (NhanVien nhanVien : dsNV) {
            modelNV.addRow(new Object[]{nhanVien.getMaNV(), nhanVien.getHoTen(), nhanVien.getGioiTinh(), nhanVien.getSDT(), nhanVien.getEmail(), nhanVien.getNgaySinh(), nhanVien.getTrinhDo()});
        }
    }
